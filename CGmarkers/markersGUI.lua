------------------------------------------------------------------------------------
--	PROYECTO:			Comunidad Gamers 2.0 (2020)
--	DERECHOS:			Todos los derechos reservados por los desarrolladores
--	ARCHIVO:			CGmarkers/markersGUI.lua
...
function crearElemento()
	...
	if (area == "Markers") then
		...
		local tam = tonumber(guiGetText(tabs[area]["tam"])) or 1.5
		elementos[area] = createMarker(tonumber(x), tonumber(y), tonumber(z), tipo, tam, 255, 0, 0, 255)
...
function obtenerPosicion()
	...
	local x, y, z = string.format("%.1f", x), string.format("%.1f", y), string.format("%.1f", z)
...
function crearElemento2(button, state, absoluteX, absoluteY, tipo)
	local area = getActiveTab() or tipo
	if (not area) then
		return false
	end
	if (elementos[area] and isElement(elementos[area])) then
	...
	if (area == "Markers") then
		local tipo = guiComboBoxGetItemText(tabs[area]["editID"], guiComboBoxGetSelected(tabs[area]["editID"]))
		local tam = tonumber(guiGetText(tabs[area]["tam"])) or 1.5
		elementos[area] = createMarker(x, y, z, tipo, 3, 255, 0, 0, 255)
	elseif (area == "Pickups") then
		if (not tonumber(tipo)) then
			outputChatBox("Debes ingresar una ID de modelo válida.", 255, 0, 0)
			return false
...
function actualizarElemento()
	...
	if (area == "Markers") then
		local tipo = guiComboBoxGetItemText(tabs[area]["editID"], guiComboBoxGetSelected(tabs[area]["editID"]))
		local tam = tonumber(guiGetText(tabs[area]["tam"])) or 1.5
		elementos[area] = createMarker(x, y, z, tipo, tam, 255, 0, 0, 255)
	elseif (area == "Pickups") then
		if (not tonumber(tipo)) then
			outputChatBox("Debes ingresar una ID de modelo válida.", 255, 0, 0)
			return false
...
addEvent("CGmarkers.openGUI", true)
addEventHandler("CGmarkers.openGUI", root, openGUI)
...
function exportarElemento()
	...
	if (area == "Markers" or area == "Pickups") then
		setClipboard(x..", "..y..", "..z)
		outputChatBox("Marker/Pickup copiado en portapapeles. (Ctrl + V)", 0, 255, 0)
	elseif (area == "Objects") then
		local rx, ry, rz = string.format("%.3f", rx), string.format("%.3f", ry), string.format("%.3f", rz)
		setClipboard(getElementModel(elementos[area])..", "..x..", "..y..", "..z..", "..rx..", "..ry..", "..rz)
		outputChatBox("Objeto copiado en portapapeles. (Ctrl + V)", 0, 255, 0)
	elseif (area == "Peds") then
		local rz = string.format("%.3f", rz)
		setClipboard(getElementModel(elementos[area])..", "..x..", "..y..", "..z..", "..rz)
		outputChatBox("Ped copiado en portapapeles. (Ctrl + V)", 0, 255, 0)
	end
end